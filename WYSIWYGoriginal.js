...
var checkbox, newwidth, newwidthd, newheight, newheightd, wWidth, pWidth, wHeight, pHeight, newslider
var storefrom, storeleft, storebox, imgURLred, imgURLgreen
var FirstOpen = true;
var external = false;
...
$(function()
{
	...
	if ($('#gbvg')[0]) { 
		$('#gbvg ol.gbtc').prepend('<li class="gbt"><div id="wysiwyg_B"><a href="#" class="wysiwyg" title="WYSIWYG"><img src='+imgURLgreen+' id="wysiwyg_green" style="height:100%; width:100%; display:none;"  /><img src='+imgURLred+' id="wysiwyg_red" style="height:100%; width:100%;"  /></a></div></li>')
		$('#wysiwyg_green').hide()
	}
	else if ($('#gbg')[0])
	{
		$('#gbg ol.gbtc').prepend('<li class="gbt"><a href="#" class="wysiwyg" title="WYSIWYG"><img src='+imgURLred+' style="height:100%; width:100%;"  /></a></li>')
	}
	else
	{
		external = true;
		
		$('.Ng').append('<div id="wysiwyg_container"><div id="wysiwyg"></div></div>')
		$('td.bw').after('<td id="wysiwyg_button"><img src='+imgURLgreen+' id="wysiwyg_green" style="height:29px; width:29px; display:none; cursor:pointer;"  /><img src='+imgURLred+' id="wysiwyg_red" style="height:29px; width:29px; cursor:pointer;"  /></td><td id="wysiwyg_stats" style="padding-left:10px; vertical-align:middle; white-space: nowrap;">Words: <span id="wysiwyg_words">0</span> Chars: <span id="wysiwyg_chars">0</span></td>')
		$('#wysiwyg_green').hide()
		$("#wysiwyg_container").scroll(function () 
		{ 
			$(".yd").scrollTop($("#wysiwyg_container").scrollTop());
		});
		$(".yd").scroll(function () 
		{ 
			$("#wysiwyg_container").scrollTop($(".yd").scrollTop());
		});	
	};		
});
...
function load()
{
	...
	$('#wysiwyg_dialog').dialog({
		...
		title: "G+ WYSIWYG",
		buttons: [{ text: "Preview", click: function() {updatePreview(StorePreview,"Preview")}},{ text: "Clear", click: function() {$('#wysiwyg').html("");$('#wysiwyg_chars').html("0");$('#wysiwyg_words').html("0");}}],
		autoOpen: false,
		open: function() {
...
function dialogposition(a,b)
{
	...
	if (a != "bypassN" && a != "bypassB")
	{
		...
		{
			fromleft = ($("#contentPane").offset().left) + $("#contentPane").width() + ($(window).scrollLeft()) - 5 - pWidth
			fromtop = ($("#gb").height()) + 1
		}
		else
		{
			fromleft = ($("#gbwc").offset().left) - ($(window).scrollLeft()) - 7 - pWidth
			if (fromleft < 0)
			{
				fromleft = storeleft
			}
		...
		$("#wysiwyg_dialog").dialog({position: [fromleft,fromtop]});
	}
	switch(a)
	{
...
{
	PostTheID(dom.find(".editable"))
	WYSIWYG(dom,"ShareBox")
}


$('html').on("mouseenter", ".editable",function(e) {
	if (!checkboxm || external){return;}
	dialogposition("normal","")
})
...
$('html').on("click", "a.wysiwyg",function(e) {
	...
	{	
		YTube = a.find("a.ot-anchor.pc").attr('href')
		if (YTube)
		{
			videourl = YTube
		...
		var OldString = a.find(PostID+".editable").html()
		...
		{		
			...
			if (!external)
			{
				...
				if (checkbox && !manualclose && !isOpen && storefrom != from)
				{
					storefrom = from
					$("#wysiwyg_dialog").dialog("open");
					var el = a.find(PostID+".editable")
					if (from == "ShareBox" || from == "Notifications" || from == "FirstIframe")
					{
					...
					else
					{
						...
						dialogposition("normal1",$(PostID).get(0))
					}
				}
			}
		}
...
function updatePreview(update,from)
{
	...
	update = update.replace(/<Rspan/gi,"<span").replace(/spanT>/gi,"span>")
	update = update.replace(/(<span id="R[0-9]+?)(".+?>)/gi,"$1T$2")
...
function strip(html)
{
	...
	return tmp.textContent||tmp.innerText;
		...
		{
			LBRe = LBRe.replace(/<(span id="(R[0-9]+?T?).+?><aRp>((\S|\s)+?)<\/aRp><\/span)>/gi,"<R$1T>")
			a = aLinks(LBRe)
